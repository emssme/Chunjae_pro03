            ...
            <div class="columns">
                ...
                <div class="row column text-center">
                    <div class="container">
                        <div>
                            <%--미답변 질문 목록--%>
                            <table class="table is-fullwidth is-center" id="notice-table">
                                <thead>
                                <tr>
                                    ...
                                    <th width="10%" class="has-text-centered">삭제</th>
                                </tr>
                                </thead>
                                <tbody>
                                <c:forEach items="${noticeList }" var="noticeList" varStatus="status">
                                    <tr>
                                        <td class="has-text-centered">${noticeList.seq}</td>
                                        <td><a href="${path}/notice/detail.do?seq=${noticeList.seq}"
                                               style="color: #000000;">${noticeList.title}</a></td>
                                        <td class="has-text-centered">
                                            <fmt:parseDate value="${noticeList.regdate}" var="regdate"
                                                           pattern="yyyy-MM-dd HH:mm:ss"/>
                                            <fmt:formatDate value="${regdate}" pattern="yyyy-MM-dd"/>
                                        </td>
                                        <td class="has-text-centered"><a href="${path}/admin/noticeDelete.do?seq=${noticeList.seq}">삭제</a></td>
                                    </tr>
                                </c:forEach>
                            ...
                            <nav class="pagination is-rounded is-centered mb-6" role="navigation" aria-label="pagination">
                                <c:if test="${curPage > 5}">
                                    <a href="${path}/admin/adminNoticeList.do?page=${page.blockStartNum - 1}"
                                       class="pagination-previous">Previous</a>
                                </c:if>
                                <c:if test="${page.blockLastNum < page.totalPageCount}">
                                    <a href="${path}/admin/adminNoticeList.do?page=${page.blockLastNum + 1}"
                                    ...
                                    <c:forEach var="i" begin="${page.blockStartNum}" end="${page.blockLastNum}">
                                        <c:choose>
                                            <c:when test="${i == curPage}">
                                                <li>
                                                    <a href="${path}/admin/adminNoticeList.do?page=${i}"
